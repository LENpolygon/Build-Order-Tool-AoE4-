...
if (isNaN(usp.get("f"))) { // Update View counter
    ...
    if (docSnap.exists()) {
        ...
        var newScore = Math.max(docData.score - 10, Math.min(docData.score + 10, docData.views + Math.pow(docData.likes * 10, 1.069) - Math.pow(Math.floor((Date.now() - docData.timestamp) / (1000 * 60 * 60 * 24)), 1.69)));
        await updateDoc(
            ref, {
